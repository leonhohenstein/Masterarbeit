...
vn <- c("flow", "cwb1", "cwb2", "cwb3", "cwb6", "cwb12","cwb24","cwb52", "month", 
        ...
        "year", "week_year","week_tot", "Tmin", "Tmax", "swe_tot","snow_added","snow_melted","swe_added","swe_melted",
        "swe_added_agg_4","swe_melted_agg_4","snow_added_agg_4","snow_melted_agg_4",
        "swe_added_agg_8","swe_melted_agg_8","snow_added_agg_8","snow_melted_agg_8" , "prec", "sun", "rET",
...
vn_lagged <- c("flow", "cwb1", "cwb2", "cwb3", "cwb6", "cwb12", "cwb24","cwb52",
               ...
               "Tmin", "Tmax", "swe_tot","snow_added","snow_melted","swe_added","swe_melted", "prec", "sun", "rET", "flow_spline", "deriv_1", "deriv_2")

vn_interaction <- c("date", "flow", "cwb1", "cwb2", "cwb3", "cwb6", "cwb12","cwb24","cwb52", "month",
                    
                    "year", "week_year","week_tot", "Tmin", "Tmax", "swe_tot","snow_added","snow_melted","swe_added","swe_melted", "prec", "sun", "rET",
                    "sin_month",   "cos_month","sin_week", "cos_week",
                       "flow_min",
                    "flow_ma2",      "diff_ma2" , "flow_ma4",
                    "diff_ma4",      "flow_ma8",      "diff_ma8",      "deriv_1",       "deriv_2",
...
model_name <- c("model_2025-06-16_reconstructed")
today <- Sys.Date()
...
coefs_df <- purrr::imap_dfr(coefs_list, ~ {
  horizon_char <- .y
  df <- .x
  df <- rownames_to_column(df, var = "variable")
  df %>%
    mutate(horizon_char = horizon_char) %>%
    rename(coefs = s1)
})


coefs_df$fc_horizon <- stringr::str_remove(coefs_df$horizon, "fc_horizon_") %>%
  as.numeric()

coefs_df$n_horizon <- coefs_df$fc_horizon %>% 
...
plot <- coefs_df %>%
  ...
  labs(
    ...
    title = paste0("Station:  | Absolute Importance of 20 Most Important Predictor Variables in ",model_name),
