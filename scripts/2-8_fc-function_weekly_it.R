...
#------------------------------------------------------------------
# 
# calculate_lags2 <- function(df, resp ,lags, h) {
# 
#   for(n in 1: min(c((horizon-1),lags)))
#   n <- 1
# flow_lags_iterative <- results[[paste0("fc_horizon_",(n))]][["model"]]
# 
# model_previous <- results[[paste0("fc_horizon_",(n))]][["model"]]
# 
#   flow_lags_iterative <- predict(model_previous, x)
# 
#   
...
#   names(map_lag) <- lags
#   
#   return(df %>% mutate(across(.cols = all_of(c(resp)), .fns = map_lag, .names="{.col}_lag_{.fn}")))
#   
# }
...
get_forecast <- function(x, resp, horizon, var_names, filter_year = 2014, var_names_lagged = NULL, lags = NULL, 
                         ...
                         rolling_window = TRUE, iterative = FALSE, fc_prev_its = NULL)
  ...
  
{
  ...
  ##### Add Iterative Forecasting Conditions ----
  ...
  if(horizon > 1 &&  !is.null(lags) && iterative == TRUE)

  {
    
    for (n in 1 : min(c(( horizon - 1), lags))) {
        # n <- 1
        flow_lag_n <- results_prev_its %>%
          filter(horizon == (h - n)) %>%
          mutate(!!sym(ifelse(n == 1,
                              "flow",
                              paste0("flow_lag_", n - 1))) := pred) %>% 
          select(c(ifelse(n == 1,
                        "flow",
                        paste0("flow_lag_", n - 1)),"date"))
    ...
    
  {
    ...
    out <- x_all[c("date", "y")] %>% mutate(pred = pred) 
