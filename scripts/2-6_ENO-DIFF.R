...
df$Flow_min_monthly_diff <- c(NA,diff(df$Flow_min_monthly))

#calculate lagged variables of differenced flow
if(lagged == TRUE){
  
  df$Flow_min_monthly_diff_lag1 <- NA
  
  for(i in 2: (nrow(df))){
    df$Flow_min_monthly_diff_lag1[i] <-  df$Flow_min_monthly_diff[i-1]
...
if(lagged == TRUE){
  ...
  df$Flow_min_monthly_diff_lag2 <- NA
  
  for(i in 3: (nrow(df))){
    df$Flow_min_monthly_diff_lag2[i] <-  df$Flow_min_monthly_diff[i-2]
...
y_diff <- df$Flow_min_monthly_diff
...
y_train_df_diff <- matrix(NA,nrow_train,10)
...
for (k in 1:10) {
  ...
  y_train_df_diff[,k] <- y_diff[train_rows]
...
#   geom_line(aes(x=Date,y=diff_values), color = "red")+
#   theme_light()
# 
# df %>%  
#   ggplot()+
...
for (k in 1:10) {
  ...
  y_train_diff <- y_train_df_diff[,k]
  y_test_original <- y_test_df_original[,k]
  
  for (i in 1:10) {
    
    temp.fit <- cv.glmnet(x_train, y_train_diff, type.measure="mse",
    ...
    predictions_undiffed <- rep(NA, length(predictions_diffed))
    
    for (j in 2:length(predictions_diffed)) {
      predictions_undiffed[1] <- NA
        
      predictions_undiffed[j] <- y_test_original[j]-predictions_diffed[j+1]
    }
    # predictions_undiffed <- (predictions + df$Flow_min_monthly_diff[test_rows]) 
    
    # y_test_trended <- (y_test + diff_values) 
    # y_test_trended <- y_test_original
    
    
    temp.predicted_df <- data.frame(q_predicted = predictions_undiffed, q_obs = y_test_original, Date=df$Date)
    ...
    mse <- hydroGOF::mse(predictions_undiffed, y_test_original)
    
    mae <- hydroGOF::mae(predictions_undiffed, y_test_original)
    
    R2 <- hydroGOF::R2(predictions_undiffed, y_test_original)
    
    mNSE <- hydroGOF::mNSE(predictions_undiffed, y_test_original, j=1)
    
    kge <- hydroGOF::KGE(predictions_undiffed, y_test_original, j=1)
    
    RMSE <- hydroGOF::rmse(predictions_undiffed, y_test_original)
