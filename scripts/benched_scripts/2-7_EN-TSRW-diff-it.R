...
for (h in fc_horizon) {
  ...
  df$flow_obs_diffs_h <- c(diff(df$Flow_min_monthly,lag = 1),rep(NA, h)) #creates the difference between the current timestemp and the timnestamp to be forecasted (h)
  
  # df$flow_obs_diffs_h <-   dplyr::lead(df$flow_obs_diffs_h, h-1)   #move the forecasted values downoards for h rows to ensure the forecasting horizont matches the forecasted month
  
  df$flow_diffs_h_lag1 <-  df$Flow_min_monthly_lag1 - df$Flow_min_monthly_lag2#creates the difference between the current timestemp and the timnestamp to be forecasted (h)
  ...
  pred_eval <- cbind(dates_eval_vars,y_obs,dates_eval_pred) %>% 
    as.data.frame()
 
  pred_eval$pred <- predict(model, newdata = x_eval) %>% 
    as.vector() 
  pred_eval
  colnames(pred_eval) <- c("Date_obs","y_obs","Date_pred","pred")
  
  pred_eval$pred_BT <- NA
  for (i in (h+1):nrow(pred_eval)) {
  # i <- 2
    # pred_eval$pred[i] <-  pred_eval$pred[i] + pred_eval$pred[i-1]
    pred_eval$pred_BT[i]  <- pred_eval$pred[i]  + pred_eval$y_obs[i-h] 
  ...
  ### iterative script part (for now not applied to the diffed model)
