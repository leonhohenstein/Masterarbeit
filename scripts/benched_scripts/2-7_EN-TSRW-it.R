...
for (h in fc_horizon) {
  ...
  if (h > 0) {
    #set the lagged flow variables to be the output from the last iterations predictions
    df$Flow_min_monthly_lag2 <- df$Flow_min_monthly_lag1
    df$Flow_min_monthly_lag1 <- next_round
    #note: the optimal lambda of the test set (evaluation) is used for predcting the lagged values 
  }
  ...
  x <- predict(preproc, x) %>% # Apply the same scaling & centering as in the model training
    as.matrix()
  
  next_round <- predict(final_model, newx = x, 
          s = best_tunes_list[[paste0("fc_horizon_",(h-1))]][["lambda"]], 
          type = "response") %>% 
    as.data.frame()
  next_round <- next_round$.[1:(length(next_round)-1)]
}
