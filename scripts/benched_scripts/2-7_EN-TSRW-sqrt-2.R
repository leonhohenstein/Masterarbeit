...
for (h in fc_horizon) {
  ...
  df$Flow_min_monthly <-   dplyr::lead(df$Flow_min_monthly, h)   #move the forecasted values downoards for h rows to ensure the forecasting horizont matches the forecasted month
  ...
  slices <- createTimeSlices(1:(nrow(x_train_sqrt) - h), initialWindow = origin,
  ...
  for (i in seq_along(slices$train)) {
    ...
    exclude_range <- seq(max_test_index, min(max_test_index + n_exclude, nrow(x_train_sqrt)), by = 1)
...
#Extra Plotting for the presetnation:



png(filename = paste0("results/",model_name,"/",model_name,"__RMSE_n_coefs-fc-horizon_",h,"-1.png"),
    width = 1000, height = 600, units = "px")
GOF_df_eval <- GOF_df_eval %>%
  mutate(scaled_RMSE_2 = (RMSE - min(RMSE)) /
           (max(RMSE) - min(RMSE)) * (max(n_coefs) - min(n_coefs)) + min(n_coefs))
...
png(filename = paste0("results/",model_name,"/",model_name,"__RMSE_n_coefs-fc-horizon_",h,"-2.png"),
    width = 1000, height = 600, units = "px")
plot <- GOF_df_eval %>%
  ggplot(aes(x = log10(lambda.value))) +
  # geom_col(aes(y = n_coefs), color = "lightgrey", alpha = 0.7) +
...
png(filename = paste0("results/",model_name,"/",model_name,"__RMSE_n_coefs-fc-horizon_",h,"-3.png"),
    width = 1000, height = 600, units = "px")
GOF_df_eval <- GOF_df_eval %>%
  mutate(scaled_RMSE_2 = (RMSE - min(RMSE)) /
           (max(RMSE) - min(RMSE)) * (max(n_coefs) - min(n_coefs)) + min(n_coefs))
