...
for (h in fc_horizon) {
 h <- 2
 #### loading of data and initial data preparation ----
...
df$Flow_min_monthly <-   dplyr::lead(df$Flow_min_monthly, h)   #move the forecasted values upwards for h rows to ensure the forecasting horizont matches the forecasted month
...
pred_eval<- pred_eval[(1+h):nrow(pred_eval),]
