...
model_name <- c("EN-TSRW-sqrt-it-shifted")
...
for (h in fc_horizon) {
  ...
  if(h > 0){
    ...
    df$Flow_min_monthly_lag1 <- pred_all[1:(length(pred_all)-h+1)]
    df$Flow_min_monthly_lag2 <-   dplyr::lead(df$Flow_min_monthly_lag2, h)   #move the target variable upwards for h rows to allow a prediction of a previous month with variables of the current time point the forecasting horizont matches the forecasted month
    
    df$Flow_min_monthly_lag2 <- lag_flow_1_previous[1:(length(pred_all)-h+1)]
    df$Flow_min_monthly_lag1 <-   dplyr::lead(df$Flow_min_monthly_lag2, h)   #move the target variable upwards for h rows to allow a prediction of a previous month with variables of the current time point the forecasting horizont matches the forecasted month
