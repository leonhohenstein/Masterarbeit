...
x_train_sqrt <- df_train %>% 
  as.data.frame() %>% 
  select(-c("Flow_min_monthly")) 
x_train_sqrt$Flow_min_monthly_lag1 <- sqrt(x_train_sqrt$Flow_min_monthly_lag1)
x_train_sqrt$Flow_min_monthly_lag2 <- sqrt(x_train_sqrt$Flow_min_monthly_lag2)


x_eval_sqrt <- df_eval %>% 
  as.data.frame() %>% 
  select(-c("Flow_min_monthly")) 
x_eval_sqrt$Flow_min_monthly_lag1 <- sqrt(x_eval_sqrt$Flow_min_monthly_lag1)
x_eval_sqrt$Flow_min_monthly_lag2 <- sqrt(x_eval_sqrt$Flow_min_monthly_lag2)
...
df_train_sqrt <- na.omit(df_train_sqrt)
...
y_obs_sqrt <- sqrt(y_obs)

pred_eval_df <- matrix(data=NA, ncol = length(fc_horizon),nrow = length(y_obs_sqrt))
for (t in fc_horizon) {
  
  x_fc <- cbind(x_fc, y_obs_sqrt)
  x_fc <- na.omit(x_fc)
  y_obs_sqrt <- x_fc$y_obs_sqrt
  x_fc <- x_fc %>% 
    select(-"y_obs_sqrt")
  x_fc <- as.matrix(x_fc)
  ...
  data_fc <- x_fc[t:nrow(x_eval_sqrt),] %>% 
    as.data.frame
...
pred_eval_df$pred_t1 <- pred_eval_df$pred_t1^2 #backtransformations
pred_eval_df$pred_t2 <- pred_eval_df$pred_t2^2
pred_eval_df$pred_t3 <- pred_eval_df$pred_t3^2
...
for (t in 1:length(fc_horizon)) {
  x_fc <- data_fc_list[[paste0("fc-horizon_",t)]][[paste0("x_fc")]] %>% 
    as.data.frame() %>% 
    as.matrix()
  colnames(x_fc) <- colnames(x_train_sqrt)
  ...
  pred_eval_lambdas <- pred_eval_lambdas^2
