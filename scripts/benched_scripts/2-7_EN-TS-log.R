...
############### TRANSFORMATION of flow variables ####################################
# png("explorative_plots/tauchenbach/Flow_min_monthly_distribution.png",
#     width = 1000, height = 600, units = "px")
# 
# df %>% 
#   ggplot(aes(x = Flow_min_monthly)) +
#   geom_histogram(aes(y = ..density..), bins = 20, fill = "steelblue", color = "black", alpha = 0.6) +
#  # geom_freqpoly(aes(y = ..density..), bins = 50, color = "darkred", size = 1.2) +
#   ggtitle("Distribution of Monthly Minimum Flow", subtitle = "") +
#   labs(x = "Monthly Minimum Flow [m³/s]", y = "Frequency") +
...
#   scale_x_continuous(breaks = seq(min(df$Flow_min_monthly), max(df$Flow_min_monthly), by = 5)) +
#   scale_y_continuous(labels = scales::percent)
# dev.off()
# 
# png("explorative_plots/tauchenbach/Flow_min_monthly_distribution_log.png",
#     width = 1000, height = 600, units = "px")
# 
# df %>% 
#   ggplot(aes(x = log(Flow_min_monthly))) +
#   geom_histogram(aes(y = ..density..), bins = 20, fill = "steelblue", color = "black", alpha = 0.6) +
#   # geom_freqpoly(aes(y = ..density..), bins = 50, color = "darkred", size = 1.2) +
#   ggtitle("Log-Transformed: Distribution of Monthly Minimum Flow", subtitle = "") +
#   labs(x = "Monthly Minimum Flow [m³/s]", y = "Frequency") +
#   theme_minimal(base_size = 15) +
#   theme(panel.grid.major = element_line(color = "gray", size = 0.2), 
#         panel.grid.minor = element_blank(),
...
# 
# 
# png("explorative_plots/tauchenbach/Flow_min_monthly_distribution_sqrt.png",
#     width = 1000, height = 600, units = "px")
# 
# df %>% 
#   ggplot(aes(x = sqrt(Flow_min_monthly))) +
#   geom_histogram(aes(y = ..density..), bins = 20, fill = "steelblue", color = "black", alpha = 0.6) +
#   # geom_freqpoly(aes(y = ..density..), bins = 100, color = "darkred", size = 1.2, alpha =0.6) +
#   ggtitle("Square-Root-Transformed: Distribution of Monthly Minimum Flow", subtitle = "") +
#   labs(x = "Monthly Minimum Flow [m³/s]", y = "Frequency") +
#   theme_minimal(base_size = 15) +
#   theme(panel.grid.major = element_line(color = "gray", size = 0.2), 
#         panel.grid.minor = element_blank(),
...
y_obs_original <- df_streched %>% 
  as.data.table()%>% 
  .[,.(y_obs_original = Flow_min_monthly[.N]), by=.id]
y_obs_original <- y_obs_original[,2] %>% 
  unlist() %>% 
  as.numeric()
...
predictions_df[,2] <- y_obs_original
...
for (iteration in 1:n_iterations) {
  ...
  y_train <- sqrt(y_observed[1:(nrow(x)-1)])
  y_test <- sqrt(y_observed[nrow(x)])
  ...
  for (y in 1:10) {
    ...
    predictions_df[iteration,(y+2)] <-
      ...
      as.vector() %>% 
      .^2 #backtransformation of the square-root
...
for (lambda in lambda_names) {
  ...
  mse <- hydroGOF::mse(y_pred, y_obs_original)
  mae <- hydroGOF::mae(y_pred, y_obs_original) 
  R2 <- hydroGOF::R2(y_pred, y_obs_original)
  mNSE <- hydroGOF::mNSE(y_pred, y_obs_original, j=1)
  kge <- hydroGOF::KGE(y_pred, y_obs_original, j=1)
  RMSE <- hydroGOF::rmse(y_pred, y_obs_original)
