...
###coefficients plots ------
  coefs_df <- data.frame()
  coefs_df <- coefs_list_fc[[paste0("fc_horizon_",h)]] %>% 
    t() %>%
    as.data.frame()
...
######  QUANTIL BASED EVALUATION ----
  ...
  quant_eval_95 <- do.call(rbind,pred_list) %>% 
    setDT()
  quant_eval_95 <- quant_eval_95[,.(pred_Q95 = as.factor(pred < quantiles$Q95),
                                    obs_Q95 = as.factor(y_obs < quantiles$Q95),
                                    Date = Date,
                                    horizon = horizon)]
...
plot <- pred_df %>% 
  ...
  geom_line(aes(y = y_obs), color = "grey", linetype = "solid", size = 1.2) +  # Darker grey for observed line
...
plot <- pred_df %>% 
  ...
  geom_line(aes(y = y_obs), color = "darkgrey", linetype = "solid", size = 1.4, alpha = 1) +  # Dark grey observed line
  labs(
    y = "Monthly Low Flow [m³/s]",
    x = "Date",
...
# coefficients varation along the fc-horizon ---------
coefs_df_final <- do.call(rbind,coefs_list_final)
coefs_df_final$fc_horizon <- as.factor(coefs_df_final$fc_horizon)
...
plot <- coefs_df_final %>% 
  setDT() %>% 
  .[value != 0] %>% 
  ggplot(aes(x = fc_horizon, y = value, fill = fc_horizon)) +
