...
model_name <- c("EN-TSRW-diffed-values")
...
for (h in fc_horizon) {
  ...
  # df$flow_obs_diffs_h <-   dplyr::lag(df$flow_obs_diffs_h, h)   #move the forecasted values downoards for h rows to ensure the forecasting horizont matches the forecasted month
  ...
  # df$Flow_min_monthly_lag2_diffed <- c(diff(df$Flow_min_monthly_lag2,lag = h),rep(NA, h)) #creates the difference between the current timestemp and the timnestamp to be forecasted (h)
  # df$Flow_min_monthly_lag1_diffed <- c(diff(df$Flow_min_monthly_lag1,lag = h),rep(NA, h)) #creates the difference between the current timestemp and the timnestamp to be forecasted (h)
  
  # df$flow_obs_diffs_h_lag1 <-   dplyr::lag(df$flow_obs_diffs_h, 1)   #move the forecasted values downoards for h rows to ensure the forecasting horizont matches the forecasted month

  # df$flow_obs_diffs_h_lag2 <-   dplyr::lag(df$flow_obs_diffs_h, 2)   #move the forecasted values downoards for h rows to ensure the forecasting horizont matches the forecasted month
  

  df$Flow_min_monthly <- NULL
