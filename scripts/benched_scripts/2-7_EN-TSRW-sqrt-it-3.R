...
model_name <- c("EN-TSRW-sqrt-it-3")
...
for (h in fc_horizon) {
  ...
  date_pred_all <- dplyr::lead(df$Date, h)
  ...
  if(h > 1){
    ...
    df$Flow_min_monthly_lag1 <- pred_all$s1[1:(length(pred_all$s1)-1)]

    df$Flow_min_monthly_lag2 <- lag_flow_1_previous[1:(length(pred_all$s1)-1)]
  ...
  pred_all <- pred_all^2 %>% 
    as.data.frame()
  pred_all$date_pred_all <- date_pred_all
  pred_all$date_var <- df$Date
