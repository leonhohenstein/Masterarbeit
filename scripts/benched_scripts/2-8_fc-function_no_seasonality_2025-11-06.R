#changes compare to Standard script:
...
interaction_terms <- c("Tmin","Tmax","prec","swe_tot","swe_added","swe_melted","swe_melted_agg_4",
                       "swe_melted_agg_8", "swe_added_agg_4","swe_added_agg_8")
...
# Initiate Main Loop and define Loop Boundaries to Iterate ----
for(i in 1:length(test_years_list)){
  ...
  n_coefs_list <- list()
  coefs_global_list <- list()
  coefs_local_list <- list()
  coefs_final_list <- list()
  GOF_list <- list()
...
for(station in stations_list){
  ...
  #                     
  #                     week_hyd_sin = sin(week_year / (52 / (2 * pi)) - 4 * 2 * pi / 52),
  #                     
  #                     week_sin_half = sin(week_year/(104/(2*pi)) -52 * 2 * pi /104),
  #                   
  #                     week_hyd_cos = cos(week_year / (52 / (2 * pi)) - 4 * 2 * pi / 52),
  #                   
  ...
  customSummary <- function(data, lev = NULL, model = NULL) {
    ...
    mare_val <- mean((abs(data$pred-data$obs))/data$obs)
    
    out <- c(MAE = mae_val, RMSE = rmse_val, MSE = mse_val, MARE = mare_val)
    return(out)
  }
    ...
    
  {
   ...
   #### extraction of lambdas ----
    ...
    m$results <- m$results %>% mutate(MSE.1se = MSE + MSESD /sqrt(k)*SE.factor)
    m$results <- m$results %>% mutate(RMSE.1se = RMSE + RMSESD /sqrt(k)*SE.factor)
    m$results <- m$results %>% mutate(MAE.1se = MAE + MAESD /sqrt(k)*SE.factor)
    
    temp <- m$results[m$results$MSE <= threshold,]
    
    lambda.1se <- temp$lambda[which.max(temp$lambda)]
    
    lambda_sel <- ifelse(lambda_opt == "lambda.1se",lambda.1se,lambda.min)  
    
    
    global_optimization_results <- m$results
    
    ### extraction of global coefs ---
      
      coefs_global <- as.vector(coef(m$finalModel, s = lambda_sel))
    names(coefs_global) <- rownames(coef(m$finalModel, s = lambda_sel))
    ...
    for (i in seq_along(slices$train)) {
      ...
      if (seasonality) {
        ...
        m_fold <- caret::train(
          ...
          tuneGrid = expand.grid(alpha = 1, lambda = lambda_sel),
          preProc = c("center", "scale"),
          metric = obj_fnct
        )
      ...
      coef_vector <- as.vector(coef(m_fold$finalModel, s = lambda_sel))
      names(coef_vector) <- rownames(coef(m_fold$finalModel, s = lambda_sel))
      fold_coefs[[paste0("Fold_", i)]] <- coef_vector
    }
    
    coefs_local <- do.call(cbind, fold_coefs) %>% as.data.frame()
    
    coef_local_means <- rowMeans(coefs_local)
    coef_local_sds <- apply(coefs_local, 1, sd)
...

    {
...

      {
        m <- caret::train(x = X, y = y, method = "glmnet", family = "gaussian",
                          ...
                          trControl = tc, tuneGrid = expand.grid(alpha = 1, lambda = lambda_sel),
    ...
    coefs_final <- as.vector(coef(m$finalModel, s = lambda_sel))
    names(coefs_final) <- rownames(coef(m$finalModel, s = lambda_sel))
    
    coefs_final <- coefs_final %>% as.data.frame()
    ...
    coefs <- coef(final_model, s = lambda_sel) %>% 
      as.matrix() %>% 
      as.data.frame()
    ...
    return(list(forecast = out,
                model = final_model,
                coefs_global = coefs_global,
                n_coefs = n_coefs,
                coefs_local = coefs_local,
                GOF = GOF,
                coef_local_means = coef_local_means,
                coef_local_sds = coef_local_sds,
                params = list(lags = lags,rolling_window = rolling_window,stepsize = stepsize,
                              ...
                              lambda_opt = lambda_opt, lambda.1se = lambda.1se, lambda_sel = lambda_sel,lambda.min = lambda.min,
                              SE.factor = SE.factor, num_CV_folds = k),
  ...
  for (h in fc_horizons) {
    ...
    coefs_global_list[[paste0(station)]][[paste0("fc_horizon_",h)]] <- results[[paste0(station)]][[paste0("fc_horizon_",h)]]$coefs_global
    
    coefs_final_list[[paste0(station)]][[paste0("fc_horizon_",h)]] <- results[[paste0(station)]][[paste0("fc_horizon_",h)]]$coefs_final
    
    coefs_local_list[[paste0(station)]][[paste0("fc_horizon_",h)]] <- results[[paste0(station)]][[paste0("fc_horizon_",h)]]$coefs_local
    
    GOF_list[[paste0(station)]][[paste0("fc_horizon_",h)]] <- results[[paste0(station)]][[paste0("fc_horizon_",h)]]$GOF
    
    print(paste0("finished ",h," / ",max(fc_horizons)," station ",station, " test period ",i," / 3"))
...
if (file.exists(paste0("results/",today,"/",model_name,"/",test_years_text,"/"))){
  ...
  dir.create(file.path(paste0("results/",today,"/",model_name,"/",test_years_text,"/")))
    ...
    save(coefs_global_list,file = paste0("results/",today,"/",model_name,"/",test_years_text,"/",model_name,"_coefs_global_list_",test_years_text,".RData"))
    save(coefs_local_list,file = paste0("results/",today,"/",model_name,"/",test_years_text,"/",model_name,"_coefs_local_list_",test_years_text,".RData"))
    save(coefs_final_list,file = paste0("results/",today,"/",model_name,"/",test_years_text,"/",model_name,"_coefs_final_list_",test_years_text,".RData"))
