...
for (h in fc_horizon) {
  ...
  pred_eval  <- df %>% #removing surplus date columns #extraction of the actaul values for the model evalutation
  ...
  pred_eval$Date <- as.Date(pred_eval$Date) #extraction of the 
  ...
  }#in this step, the predictions are backatransformed (BT) 
  #since _diff is the difference between the value of the current time step [i-h] and 
  #the time step to be forecasted,adding the diff value to the absolute measured value
  #gives us the forecasted absolute value of the forecasted time step [i]
  ...
  pred_eval$pred <- dplyr::lead(pred_eval$pred, n = h) 
  #@Johannes meiner Meinung nach müsste man diesen schritt nicht machen aber 
  #es verbessert das ergebnis deutlich (R2 von 0.4 auf 0.8)
  #die frage ist, ob dadurch nicht einfach die forecasts auf den zeitpunkt der beobachtung zurück gerückt werden
  #dadurch würde quasi die beobachtung selbst erneut geforecasted werden und man würde nicht wirklich in die zukunft gehen
