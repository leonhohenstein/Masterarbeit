...
vn_interaction <- c("flow", "cwb1", "cwb2", "cwb3", "cwb6", "cwb12", "cwb24","cwb52", "month", 
  ...
  
{
  ...
  {
    interaction_terms <- paste(names(xtrain), "* season", collapse = " + ")  
  ...
  {
    out <- xtest[c("date",resp)] %>% mutate(pred = pred)
...
model_name <- c("EN_weekly_seasonal")
